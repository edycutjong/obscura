...
// Re-verifies a REAL Barretenberg UltraHonk invoice proof against the deployed
// Soroban contract's verify_invoice_proof via read-only simulation (no wallet,
// no fee). Real proof => true; byte-tampered public inputs => false. Makes the
// on-chain ZK verification witnessable in the browser, not just the CLI.
...
async function verifyOnChain(proofHex: string, publicInputsHex: string): Promise<boolean> {
...
export async function GET() {
  ...
  try {
    ...
    return NextResponse.json({
      ...
      note: 'Real Barretenberg UltraHonk invoice proof, re-verified live on the deployed Soroban contract by read-only simulation. Tampered public inputs are rejected.',
    });
  } catch (err) {
    return NextResponse.json(
