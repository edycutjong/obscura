// End-to-end REAL proving demo for Obscura's private invoice settlement:
//   Noir circuit (range + due-date + Poseidon2 commitment/nullifier)
//   -> nargo execute -> Barretenberg UltraHonk proof (keccak oracle)
//   -> on-chain verify_invoice_proof on the deployed Soroban contract (real
...
const CIRC = resolve(ROOT, 'circuit');
...
run('bb', [
  ...
  'keccak',
  '--bytecode_path',
  `${TARGET}/obscura_circuit.json`,
  '--witness_path',
  `${TARGET}/obscura_circuit.gz`,
  '--output_path',
  TARGET,
  '--output_format',
  'bytes_and_fields',
...
console.log(`Submitting on-chain verify_invoice_proof to ${ID} ...`);
const out = run(
  'stellar',
  [
    'contract',
...
console.log('on-chain verify_invoice_proof =>', result);
if (result !== 'true') process.exit(1);

// negative control: tamper due_date field -> must be rejected
...
console.log('on-chain verify_invoice_proof (tampered) =>', tampered);
if (tampered === 'true') {
