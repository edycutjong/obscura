...
const ID = process.env.OBSCURA_MULTINET_CONTRACT_ID || "CCAFMYQOOKT4VRRBJK2DVZBY3VHX5IAD2A3UYJCEE2WVYRYIRO4MIJED";

const run = (cmd, args, opts = {}) =>
  execFileSync(cmd, args, { encoding: "utf8", env: ENV, cwd: CIRC, stdio: ["ignore", "pipe", "inherit"], ...opts });
...
run("bb", ["prove", "--scheme", "ultra_honk", "--oracle_hash", "keccak",
  "--bytecode_path", `${TARGET}/multinet_circuit.json`, "--witness_path", `${TARGET}/multinet_circuit.gz`,
  "--output_path", TARGET, "--output_format", "bytes_and_fields"]);
run("bb", ["write_vk", "--scheme", "ultra_honk", "--oracle_hash", "keccak",
  "--bytecode_path", `${TARGET}/multinet_circuit.json`, "--output_path", TARGET, "--output_format", "bytes_and_fields"]);

console.log("off-chain verify:",
  run("bb", ["verify", "--scheme", "ultra_honk", "--oracle_hash", "keccak",
    "--proof_path", `${TARGET}/proof`, "--vk_path", `${TARGET}/vk`,
    "--public_inputs_path", `${TARGET}/public_inputs`]).includes("verified successfully") || "(see output)");
...
const out = run("stellar", ["contract", "invoke", "--id", ID, "--source", "deployer",
  "--network", "testnet", "--", "verify_netting_proof",
  "--proof", proof, "--public_inputs", publicInputs], { stdio: ["ignore", "pipe", "pipe"] });
...
const out2 = run("stellar", ["contract", "invoke", "--id", ID, "--source", "deployer",
  "--network", "testnet", "--", "verify_netting_proof",
  "--proof", proof, "--public_inputs", bad], { stdio: ["ignore", "pipe", "pipe"] });
