...
const ID =
  process.env.OBSCURA_NETTING_CONTRACT_ID || 'CBNM47QOFFPWY4HVIIVF5TSLQ65J7TZB54JX7FR7KKYEQMYYUOJHSOA2';
...
run('bb', ['prove', '--scheme', 'ultra_honk', '--oracle_hash', 'keccak',
  '--bytecode_path', `${TARGET}/netting_circuit.json`, '--witness_path', `${TARGET}/netting_circuit.gz`,
  '--output_path', TARGET, '--output_format', 'bytes_and_fields']);
run('bb', ['write_vk', '--scheme', 'ultra_honk', '--oracle_hash', 'keccak',
  '--bytecode_path', `${TARGET}/netting_circuit.json`, '--output_path', TARGET, '--output_format', 'bytes_and_fields']);
